...
from infrastructure.packaging.resource_locator import locate_content_root
from infrastructure.qt.qml_path_adapter import PathService
from ui.component.shell.workspace_orchestrator import WorkspaceViewOrchestrator
...
class ShellAdapter(QObject):
    ...
    def __init__(
        self,
        filter_storage_path: Path | None = None,
        content_root: Path | None = None,
        *,
        dialogs: QtDialogGateway | None = None,
        runtime_paths: RuntimePaths | None = None,
        documents: DocumentCollection | None = None,
        components: WorkspaceComponents | None = None,
    ) -> None:
        super().__init__()
        self._status_message = "Ready"
        self._content_root = content_root or locate_content_root()
        self._dialogs = dialogs or QtDialogGateway()
        self._runtime_paths = runtime_paths or RuntimePaths.discover()

        if components is None:
            raise ValueError("ShellAdapter must be constructed by DependencyContainer with workspace components.")
        self._workspace = components
        self._files = components.files
        self._project_tree = components.project_tree
        self._folder_scan = components.folder_scan
        self._filters = components.filters_model
        self._editing_rules = components.editing_rules
        self._suggestion_keys = components.suggestion_keys
        ...
        self._imported_filter_repository = components.imported_filter_repository
        self._log_import_errors("Imported filter", components.imported_filter_errors)
        self.filters = components.filters
        self.suggestions = components.suggestions
        self.macros = components.macros
        ...
        self._macro_step_registry = default_macro_step_registry()
    ...
    def openFileDialog(self) -> int:
        return self._add_paths_from_user(self._dialogs.choose_json_files())

    @Slot(result=int)
    def openFolderDialog(self) -> int:
        selected_folder = self._dialogs.choose_folder()
        return self.scan_folder_paths([selected_folder]) if selected_folder is not None else 0
    ...
    def importFilter(self) -> bool:
        selected_path = self._dialogs.choose_filter_import()
        if selected_path is None:
            return False
        ok, message = self.filters.import_user_filter(selected_path, self._imported_filter_repository)
    ...
    def exportFilter(self, index: int) -> bool:
        selected_path = self._dialogs.choose_filter_export()
        if selected_path is None:
            return False
        ok, message = self.filters.export_filter(index, selected_path)
        self._set_status(message)
        return ok
    ...
    def importMacro(self) -> bool:
        selected_path = self._dialogs.choose_macro_import()
        if selected_path is None:
            return False
        ok, message = self.macros.import_macro(selected_path, self._filters.filters())
        self._set_status(message)
        if ok:
    ...
    def exportMacro(self, index: int) -> bool:
        selected_path = self._dialogs.choose_macro_export()
        if selected_path is None:
            return False
        ok, message = self.macros.export_macro(index, selected_path)
        self._set_status(message)
        return ok
    ...
    def _execute_macro_step(self, step: MacroStep) -> None:
        self._macro_step_registry.execute(step, self)

    # MacroOperationPort implementation. The registry owns dispatch; the shell
    # only adapts existing feature operations and presentation notifications.
    def apply_filter_macro_step(self, step: MacroStep) -> None:
        ...
        if filter_index < 0 or not self.filters.apply_filter(filter_index, self._files):
    ...
    def dynamic_filter_macro_step(self, step: MacroStep) -> None:
        operations = {
            "page-name": (self.applyDynamicPageNameFilter, "Page name"),
            "visual-type": (self.applyDynamicVisualTypeFilter, "Visual type"),
            "visual-name": (self.applyDynamicVisualNameFilter, "Visual name"),
        }
        operation = operations.get(step.filter_name)
        if operation is None:
            raise ValueError(f"Unknown dynamic filter kind '{step.filter_name}'.")
        method, label = operation
        if not method(str(step.value)):
            raise ValueError(f"{label} filter could not be applied.")
    ...
    def _document_snapshots(self) -> dict[str, tuple[str, str, bool]]:
        return self._history_recorder.capture(self._files.collection.documents())

    def _history_file_changes(self, before: dict[str, tuple[str, str, bool]]) -> list[FileSnapshot]:
        return self._history_recorder.changes(before, self._files.collection.documents())
    ...
    def _refresh_search_results(self) -> None:
        self._workspace.search_results.reset_from_files(self._files)

    def _refresh_project_tree(self) -> None:
        self._workspace.project_tree.reset_from_files(self._files)
        self.projectTreeChanged.emit()
        self.currentDocumentChanged.emit()

    def _refresh_suggestions_now(self) -> None:
        self._workspace_orchestrator.refresh_suggestions_now()

    def _schedule_suggestions_refresh(self) -> None:
        self._workspace_orchestrator.schedule_suggestions_refresh()

    def _refresh_macro_validation(self) -> None:
        self._workspace_orchestrator.refresh_macro_validation()
        self.macrosChanged.emit()
